(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();const V={mosque:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  `,document.body.appendChild(a),(i=document.getElementById("abdest-toast-close"))==null||i.addEventListener("click",()=>{a.classList.remove("abdest-toast--show"),setTimeout(()=>a.remove(),400)}),setTimeout(()=>{document.getElementById("abdest-toast")&&(a.classList.remove("abdest-toast--show"),setTimeout(()=>a.remove(),400))},1e4)}function Lt(e){G&&clearInterval(G);const t=()=>{const a=Ke(e),i=document.getElementById("prayer-countdown"),n=document.getElementById("prayer-next-name");i&&(i.textContent=Oe(a.time)),n&&(n.textContent=`${a.icon} ${a.name}`);const r=ye(a.time),s=document.getElementById("abdest-reminder-bar");s&&H&&(r<=15&&r>0?(s.style.display="flex",s.querySelector(".abdest-bar__text").textContent=`🚿 Abdest vaktidir — ${a.name} namazına ${r} dk kaldı`):s.style.display="none"),$t(e)};t(),G=setInterval(t,1e3)}function Tt(e=24){return Array.from({length:e},(t,a)=>`<span class="waveform-bar" style="animation-delay:${(a*.08).toFixed(2)}s"></span>`).join("")}async function Te(){var y;const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page">
...
(${n.reference})`};if(navigator.share)try{await navigator.share(s)}catch{}else await navigator.clipboard.writeText(s.text),k("Hadis panoya kopyalandı!","success")})}catch{e.innerHTML='<div class="page text-center"><p class="text-danger">Bağlantı hatası: İçerikler yüklenemedi.</p></div>'}}async function qt(){const e=document.getElementById("page-content");e.innerHTML=`
  ...
  `,document.getElementById("kuran-back-btn").addEventListener("click",()=>{window.location.hash="ilim"});let a=[],i=[],n=-1,r=!1;const s=document.getElementById("kuran-audio"),u=document.getElementById("btn-play-pause");try{a=(await(await fetch("https://api.alquran.cloud/v1/surah")).json()).data;const f=document.getElementById("surah-selector");f.innerHTML=a.map(c=>`<option value="${c.number}">${c.number}. ${c.englishName} (${c.name})</option>`).join(""),document.getElementById("kuran-initial-loader").style.display="none",document.getElementById("kuran-content").style.display="block",f.addEventListener("change",c=>{l(parseInt(c.target.value))}),s.addEventListener("ended",()=>{n<i.length-1?o(n+1):v()}),s.addEventListener("play",()=>{r=!0,m(),y(n)}),s.addEventListener("pause",()=>{r=!1,m(),y(n)}),u.addEventListener("click",()=>{n===-1?i.length>0&&o(0):r?s.pause():s.play()}),document.getElementById("btn-prev-ayah").addEventListener("click",()=>{n>0&&o(n-1)}),document.getElementById("btn-next-ayah").addEventListener("click",()=>{n<i.length-1&&o(n+1)}),l(1)}catch(d){console.error("Kuran loading error",d),e.innerHTML=`<div class="page text-center"><p class="text-danger">Kur'an verileri yüklenemedi. Lütfen internet bağlantınızı kontrol edin.</p></div>`}async function l(d){v(),n=-1,document.getElementById("kuran-player").classList.remove("visible");const g=document.getElementById("verses-list");g.innerHTML='<div class="kuran-loading"><div class="kuran-loading-spinner"></div><p>Ayetler yükleniyor...</p></div>';try{const c=await(await fetch(`https://api.alquran.cloud/v1/surah/${d}/editions/quran-uthmani,tr.diyanet,ar.alafasy`)).json(),b=c.data[0],p=c.data[1],h=c.data[2],x=a[d-1];document.getElementById("surah-ar-name").textContent=x.name,document.getElementById("surah-tr-name").textContent=x.englishNameTranslation,document.getElementById("surah-revelation").textContent=x.revelationType==="Meccan"?"Mekki":"Medeni",document.getElementById("surah-ayah-count").textContent=x.numberOfAyahs,document.getElementById("player-surah-name").textContent=x.englishName,i=b.ayahs.map((E,_)=>({numberInSurah:E.numberInSurah,textAr:E.text,textTr:p.ayahs[_].text,audio:h.ayahs[_].audio})),g.innerHTML=i.map((E,_)=>`
  ...
  `}const Ot="623184133319-q9vipsqbpf4cl15skb6v6g4b9rku9med.apps.googleusercontent.com";function Ft(){const e=document.getElementById("page-content");e.innerHTML=`
  ...
  `;try{const t=await w("/auth/profile/",{requireAuth:!0});if(t._error){window.location.hash="#auth";return}e.innerHTML=`
    ...
    `,document.getElementById("logout-btn").addEventListener("click",()=>{qe(),k("Güle güle! Tekrar görüşmek üzere.","info"),window.location.hash="#auth"})}catch{e.innerHTML=`<div class="page text-center"><p class="text-danger">Bağlantı hatası: Profil yüklenemedi.</p>
