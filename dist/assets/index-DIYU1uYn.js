(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();const V={mosque:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  </svg>`},Ze="https://caferi-toplumu.onrender.com",Ue="http://localhost:8000",Ve=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?`${Ue}/api`:`${Ze}/api`;function Xe(e=!1){const t={"Content-Type":"application/json"};if(e){const a=localStorage.getItem("access_token");a&&(t.Authorization=`Bearer ${a}`)}return t}async function _(e,t={}){const a=`${Ve}${e}`,i={method:t.method||"GET",headers:Xe(t.requireAuth),...t};t.body&&(i.body=JSON.stringify(t.body));try{const n=await fetch(a,i);if(n.status===204)return{success:!0};const r=await n.json();return n.status===401&&t.requireAuth?(Pe(),{_error:!0,message:"Oturum süresi doldu"}):n.ok?r:{_error:!0,...r,status:n.status}}catch(n){return console.error("API Error:",n),{_error:!0,message:"Bağlantı xətası!"}}}async function he(e,t){const a=await _("/auth/login/",{method:"POST",body:{email:e,password:t}});if(!a._error){localStorage.setItem("access_token",a.access),localStorage.setItem("refresh_token",a.refresh);try{const i=JSON.parse(atob(a.access.split(".")[1]));localStorage.setItem("user_email",i.email),localStorage.setItem("user_role",i.role)}catch{}return!0}return!1}async function Je(e){const t=await _("/auth/login/google/",{method:"POST",body:{credential:e}});if(!t._error){localStorage.setItem("access_token",t.access),localStorage.setItem("refresh_token",t.refresh);try{const a=JSON.parse(atob(t.access.split(".")[1]));localStorage.setItem("user_email",a.email),localStorage.setItem("user_role",a.role)}catch{}return{success:!0,created:t.created}}return{success:!1}}async function We(e){return await _("/auth/register/",{method:"POST",body:e})}function Pe(){localStorage.removeItem("access_token"),localStorage.removeItem("refresh_token"),localStorage.removeItem("user_email"),localStorage.removeItem("user_role"),window.dispatchEvent(new Event("authChange"))}function $(){return!!localStorage.getItem("access_token")}function k(e,t="info",a=3e3){let i=document.querySelector(".toast-container");i||(i=document.createElement("div"),i.className="toast-container",document.body.appendChild(i));const n=document.createElement("div");n.className=`toast toast--${t}`;const r={success:"✅",error:"❌",info:"ℹ️"};n.innerHTML=`
    <span class="toast__icon">${r[t]||"🔔"}</span>
    <span class="toast__message">${e}</span>
    <button class="toast__close">✕</button>
  `,i.appendChild(n);const s=()=>{n.style.opacity="0",n.style.transform="translateY(-20px)",setTimeout(()=>n.remove(),300)};n.querySelector(".toast__close").addEventListener("click",s),setTimeout(s,a)}function Qe(){const e=localStorage.getItem("theme")||"dark";return document.documentElement.setAttribute("data-theme",e),e}Qe();function He(){const e=document.getElementById("app-header"),t=$()?"var(--color-primary)":"var(--text-muted)",i=(document.documentElement.getAttribute("data-theme")||"dark")==="light"?'<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>':'<circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>';e.innerHTML=`
  ...
  `,document.getElementById("theme-toggle").addEventListener("click",()=>{const n=document.documentElement,s=n.getAttribute("data-theme")==="light"?"dark":"light";n.setAttribute("data-theme",s),localStorage.setItem("theme",s),document.getElementById("theme-icon").innerHTML=s==="light"?'<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>':'<circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>'})}const et=[{id:"prayer",label:"Namaz",icon:V.mosque},{id:"community",label:"Topluluk",icon:V.community},{id:"ilim",label:"İlim",icon:V.book},{id:"bagis",label:"Bağış",icon:V.heart},{id:"ziyaret",label:"Ziyaret",icon:V.compass}];function tt(e,t){const a=document.getElementById("bottom-nav");a.innerHTML=et.map(i=>`
    <button class="nav-item ${e===i.id?"active":""}" data-page="${i.id}" id="nav-${i.id}">
      ${i.icon}
      <span>${i.label}</span>
    </button>
  `).join(""),a.querySelectorAll(".nav-item").forEach(i=>{i.addEventListener("click",()=>{const n=i.dataset.page;t(n)})})}const ye=Math.PI/180,oe=180/Math.PI,fe={Jafari:{name:"Shia Ithna-Ashari",fajrAngle:16,ishaAngle:14,maghribAngle:4,midnight:"Jafari"},ISNA:{name:"Islamic Society of North America",fajrAngle:15,ishaAngle:15,maghribAngle:0,midnight:"Standard"},MWL:{name:"Muslim World League",fajrAngle:18,ishaAngle:17,maghribAngle:0,midnight:"Standard"}};function at(e,t,a){t<=2&&(e-=1,t+=12);const i=Math.floor(e/100),n=2-i+Math.floor(i/4);return Math.floor(365.25*(e+4716))+Math.floor(30.6001*(t+1))+a+n-1524.5}function re(e){const t=e-2451545,a=ce(357.529+.98560028*t),i=ce(280.459+.98564736*t),n=ce(i+1.915*P(a)+.02*P(2*a)),r=23.439-36e-8*t,s=lt(se(r)*P(n),se(n))/15,u=i/15-le(s);return{declination:rt(P(r)*P(n)),equation:u}}function Re(e,t){return le(12-t)}function R(e,t,a,i){const n=re(Q+t).declination,r=Re(t,re(Q+t).equation),s=1/15*st((-P(e)-P(n)*P(a))/(se(n)*se(a)));return r+(i==="ccw"?-s:s)}function nt(e,t,a){const i=re(Q+t).declination,n=-ot(1/(e+it(Math.abs(a-i))));return R(n,t,a,"cw")}function P(e){return Math.sin(e*ye)}function se(e){return Math.cos(e*ye)}function it(e){return Math.tan(e*ye)}function rt(e){return Math.asin(e)*oe}function st(e){return Math.acos(e)*oe}function ot(e){return Math.atan(e)*oe}function lt(e,t){return Math.atan2(e,t)*oe}function ce(e){return Ke(e,360)}function le(e){return Ke(e,24)}function Ke(e,t){return e=e-t*Math.floor(e/t),e<0?e+t:e}let Q;function dt(e,t,a,i,n="Jafari"){const r=fe[n]||fe.Jafari,s=e.getFullYear(),u=e.getMonth()+1,d=e.getDate();Q=at(s,u,d)-a/360;let o={imsak:5,fajr:5,sunrise:6,dhuhr:12,asr:13,sunset:18,maghrib:18,isha:18};for(let p=0;p<3;p++)o=ct(o,t,r);const v=i-a/15;for(const p in o)o[p]+=v;if(o.imsak=o.fajr-10/60,r.midnight==="Jafari"){const p=xe(o.sunset,o.fajr);o.midnight=o.sunset+p/2}else{const p=xe(o.sunset,o.sunrise);o.midnight=o.sunset+p/2}return o}function ct(e,t,a){const i=n=>re(Q+n);return{imsak:R(a.fajrAngle+.5,e.imsak,t,"ccw"),fajr:R(a.fajrAngle,e.fajr,t,"ccw"),sunrise:R(be(),e.sunrise,t,"ccw"),dhuhr:Re(e.dhuhr,i(e.dhuhr).equation),asr:nt(1,e.asr,t),sunset:R(be(),e.sunset,t,"cw"),maghrib:R(a.maghribAngle,e.maghrib,t,"cw"),isha:R(a.ishaAngle,e.isha,t,"cw")}}function be(){return .833+.0347*Math.sqrt(0)}function xe(e,t){return le(t-e)}function j(e){if(isNaN(e))return"--:--";e=le(e);const t=Math.floor(e),a=Math.floor((e-t)*60);return`${String(t).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function ke(e,t,a,i){const n=dt(e,t,a,i,"Jafari");return{Fajr:j(n.fajr),Sunrise:j(n.sunrise),Dhuhr:j(n.dhuhr),Asr:j(n.asr),Sunset:j(n.sunset),Maghrib:j(n.maghrib),Isha:j(n.isha),Imsak:j(n.imsak),Midnight:j(n.midnight)}}function ut(e){const t=e.getFullYear(),a=e.getMonth()+1,i=e.getDate();let n=t,r=a;r<=2&&(n--,r+=12);const s=Math.floor(n/100),u=2-s+Math.floor(s/4),d=Math.floor(365.25*(n+4716))+Math.floor(30.6001*(r+1))+i+u-1524.5,o=Math.floor(d)-1948440+10632,v=Math.floor((o-1)/10631),p=o-10631*v+354,y=Math.floor((10985-p)/5316)*Math.floor(50*p/17719)+Math.floor(p/5670)*Math.floor(43*p/15238),g=p-Math.floor((30-y)/15)*Math.floor(17719*y/50)-Math.floor(y/16)*Math.floor(15238*y/43)+29,l=Math.floor(24*g/709),h=g-Math.floor(709*l/24),c=30*v+y-30,b=["Muharrem","Safer","Rebiülevvel","Rebiülahir","Cemaziyelevvel","Cemaziyelahir","Recep","Şaban","Ramazan","Şevval","Zilkade","Zilhicce"],m=["مُحَرَّم","صَفَر","رَبيع الأوَّل","رَبيع الثاني","جُمادى الأولى","جُمادى الآخرة","رَجَب","شَعبان","رَمَضان","شَوَّال","ذو القِعدة","ذو الحِجَّة"];return{day:Math.max(1,Math.min(30,h)),month:l,year:c,monthName:b[l-1]||"",monthNameAr:m[l-1]||""}}const Oe={"2026-04-05":{Fajr:"04:53",Sunrise:"06:20",Dhuhr:"12:56",Asr:"16:32",Maghrib:"19:23",Isha:"20:44",hijri:"17 Şevval 1447"},"2026-04-06":{Fajr:"04:51",Sunrise:"06:18",Dhuhr:"12:56",Asr:"16:33",Maghrib:"19:24",Isha:"20:45",hijri:"18 Şevval 1447"},"2026-04-07":{Fajr:"04:50",Sunrise:"06:17",Dhuhr:"12:56",Asr:"16:33",Maghrib:"19:25",Isha:"20:46",hijri:"19 Şevval 1447"},"2026-04-08":{Fajr:"04:48",Sunrise:"06:15",Dhuhr:"12:56",Asr:"16:33",Maghrib:"19:26",Isha:"20:48",hijri:"20 Şevval 1447"},"2026-04-09":{Fajr:"04:46",Sunrise:"06:14",Dhuhr:"12:55",Asr:"16:34",Maghrib:"19:27",Isha:"20:49",hijri:"21 Şevval 1447"},"2026-04-10":{Fajr:"04:44",Sunrise:"06:12",Dhuhr:"12:55",Asr:"16:34",Maghrib:"19:28",Isha:"20:50",hijri:"22 Şevval 1447"},"2026-04-11":{Fajr:"04:42",Sunrise:"06:11",Dhuhr:"12:55",Asr:"16:34",Maghrib:"19:29",Isha:"20:52",hijri:"23 Şevval 1447"},"2026-04-12":{Fajr:"04:40",Sunrise:"06:09",Dhuhr:"12:55",Asr:"16:35",Maghrib:"19:30",Isha:"20:53",hijri:"24 Şevval 1447"},"2026-04-13":{Fajr:"04:38",Sunrise:"06:07",Dhuhr:"12:54",Asr:"16:35",Maghrib:"19:31",Isha:"20:54",hijri:"25 Şevval 1447"},"2026-04-14":{Fajr:"04:37",Sunrise:"06:06",Dhuhr:"12:54",Asr:"16:35",Maghrib:"19:32",Isha:"20:55",hijri:"26 Şevval 1447"},"2026-04-15":{Fajr:"04:35",Sunrise:"06:04",Dhuhr:"12:54",Asr:"16:36",Maghrib:"19:33",Isha:"20:57",hijri:"27 Şevval 1447"},"2026-04-16":{Fajr:"04:33",Sunrise:"06:03",Dhuhr:"12:54",Asr:"16:36",Maghrib:"19:34",Isha:"20:58",hijri:"28 Şevval 1447"},"2026-04-17":{Fajr:"04:31",Sunrise:"06:01",Dhuhr:"12:53",Asr:"16:36",Maghrib:"19:35",Isha:"20:59",hijri:"29 Şevval 1447"},"2026-04-18":{Fajr:"04:29",Sunrise:"06:00",Dhuhr:"12:53",Asr:"16:36",Maghrib:"19:36",Isha:"21:01",hijri:"1 Zilkade 1447"},"2026-04-19":{Fajr:"04:27",Sunrise:"05:59",Dhuhr:"12:53",Asr:"16:37",Maghrib:"19:37",Isha:"21:02",hijri:"2 Zilkade 1447"},"2026-04-20":{Fajr:"04:26",Sunrise:"05:57",Dhuhr:"12:53",Asr:"16:37",Maghrib:"19:38",Isha:"21:03",hijri:"3 Zilkade 1447"},"2026-04-21":{Fajr:"04:24",Sunrise:"05:56",Dhuhr:"12:52",Asr:"16:37",Maghrib:"19:39",Isha:"21:05",hijri:"4 Zilkade 1447"},"2026-04-22":{Fajr:"04:22",Sunrise:"05:54",Dhuhr:"12:52",Asr:"16:38",Maghrib:"19:40",Isha:"21:06",hijri:"5 Zilkade 1447"},"2026-04-23":{Fajr:"04:20",Sunrise:"05:53",Dhuhr:"12:52",Asr:"16:38",Maghrib:"19:41",Isha:"21:08",hijri:"6 Zilkade 1447"},"2026-04-24":{Fajr:"04:18",Sunrise:"05:51",Dhuhr:"12:52",Asr:"16:38",Maghrib:"19:42",Isha:"21:09",hijri:"7 Zilkade 1447"},"2026-04-25":{Fajr:"04:17",Sunrise:"05:50",Dhuhr:"12:52",Asr:"16:38",Maghrib:"19:43",Isha:"21:10",hijri:"8 Zilkade 1447"},"2026-04-26":{Fajr:"04:15",Sunrise:"05:49",Dhuhr:"12:51",Asr:"16:39",Maghrib:"19:44",Isha:"21:12",hijri:"9 Zilkade 1447"},"2026-04-27":{Fajr:"04:13",Sunrise:"05:47",Dhuhr:"12:51",Asr:"16:39",Maghrib:"19:45",Isha:"21:13",hijri:"10 Zilkade 1447"},"2026-04-28":{Fajr:"04:11",Sunrise:"05:46",Dhuhr:"12:51",Asr:"16:39",Maghrib:"19:46",Isha:"21:15",hijri:"11 Zilkade 1447"},"2026-04-29":{Fajr:"04:10",Sunrise:"05:45",Dhuhr:"12:51",Asr:"16:39",Maghrib:"19:47",Isha:"21:16",hijri:"12 Zilkade 1447"},"2026-04-30":{Fajr:"04:08",Sunrise:"05:43",Dhuhr:"12:51",Asr:"16:40",Maghrib:"19:48",Isha:"21:17",hijri:"13 Zilkade 1447"},"2026-05-01":{Fajr:"04:06",Sunrise:"05:42",Dhuhr:"12:51",Asr:"16:40",Maghrib:"19:49",Isha:"21:19",hijri:"14 Zilkade 1447"},"2026-05-02":{Fajr:"04:04",Sunrise:"05:41",Dhuhr:"12:51",Asr:"16:40",Maghrib:"19:50",Isha:"21:20",hijri:"15 Zilkade 1447"},"2026-05-03":{Fajr:"04:03",Sunrise:"05:40",Dhuhr:"12:51",Asr:"16:40",Maghrib:"19:51",Isha:"21:22",hijri:"16 Zilkade 1447"},"2026-05-04":{Fajr:"04:01",Sunrise:"05:39",Dhuhr:"12:50",Asr:"16:41",Maghrib:"19:52",Isha:"21:23",hijri:"17 Zilkade 1447"},"2026-05-05":{Fajr:"03:59",Sunrise:"05:37",Dhuhr:"12:50",Asr:"16:41",Maghrib:"19:53",Isha:"21:25",hijri:"18 Zilkade 1447"},"2026-05-06":{Fajr:"03:58",Sunrise:"05:36",Dhuhr:"12:50",Asr:"16:41",Maghrib:"19:54",Isha:"21:26",hijri:"19 Zilkade 1447"}},mt=["Muharrem","Safer","Rebiülevvel","Rebiülahir","Cemaziyelevvel","Cemaziyelahir","Recep","Şaban","Ramazan","Şevval","Zilkade","Zilhicce"],pt=["مُحَرَّم","صَفَر","رَبيع الأوَّل","رَبيع الثاني","جُمادى الأولى","جُمادى الآخرة","رَجَب","شَعبان","رَمَضان","شَوَّال","ذو القِعدة","ذو الحِجَّة"];function yt(e){const t=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}-${String(e.getDate()).padStart(2,"0")}`,a=Oe[t];if(!a)return null;const i=a.hijri.split(" "),n=parseInt(i[0]),r=i[1],s=parseInt(i[2]),u=mt.indexOf(r);return{timings:{Fajr:a.Fajr,Sunrise:a.Sunrise,Dhuhr:a.Dhuhr,Asr:a.Asr,Maghrib:a.Maghrib,Isha:a.Isha},hijri:{day:n,month:u+1,year:s,monthName:r,monthNameAr:pt[u]||""},isStatic:!0}}const gt="modulepreload",vt=function(e){return"/"+e},we={},ht=function(t,a,i){let n=Promise.resolve();if(a&&a.length>0){let s=function(o){return Promise.all(o.map(v=>Promise.resolve(v).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};document.getElementsByTagName("link");const u=document.querySelector("meta[property=csp-nonce]"),d=(u==null?void 0:u.nonce)||(u==null?void 0:u.getAttribute("nonce"));n=s(a.map(o=>{if(o=vt(o),o in we)return;we[o]=!0;const v=o.endsWith(".css"),p=v?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${o}"]${p}`))return;const y=document.createElement("link");if(y.rel=v?"stylesheet":gt,v||(y.as="script"),y.crossOrigin="",y.href=o,d&&y.setAttribute("nonce",d),document.head.appendChild(y),v)return new Promise((g,l)=>{y.addEventListener("load",g),y.addEventListener("error",()=>l(new Error(`Unable to preload CSS for ${o}`)))})}))}function r(s){const u=new Event("vite:preloadError",{cancelable:!0});if(u.payload=s,window.dispatchEvent(u),!u.defaultPrevented)throw s}return n.then(s=>{for(const u of s||[])u.status==="rejected"&&r(u.reason);return t().catch(r)})};/*! Capacitor: https://capacitorjs.com/ - MIT License */var Y;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Y||(Y={}));class ue extends Error{constructor(t,a,i){super(t),this.message=t,this.code=a,this.data=i}}const ft=e=>{var t,a;return e!=null&&e.androidBridge?"android":!((a=(t=e==null?void 0:e.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},bt=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},i=a.Plugins=a.Plugins||{},n=()=>t!==null?t.name:ft(e),r=()=>n()!=="web",s=p=>{const y=o.get(p);return!!(y!=null&&y.platforms.has(n())||u(p))},u=p=>{var y;return(y=a.PluginHeaders)===null||y===void 0?void 0:y.find(g=>g.name===p)},d=p=>e.console.error(p),o=new Map,v=(p,y={})=>{const g=o.get(p);if(g)return console.warn(`Capacitor plugin "${p}" already registered. Cannot register plugins twice.`),g.proxy;const l=n(),h=u(p);let c;const b=async()=>(!c&&l in y?c=typeof y[l]=="function"?c=await y[l]():c=y[l]:t!==null&&!c&&"web"in y&&(c=typeof y.web=="function"?c=await y.web():c=y.web),c),m=(z,B)=>{var L,q;if(h){const D=h==null?void 0:h.methods.find(S=>B===S.name);if(D)return D.rtype==="promise"?S=>a.nativePromise(p,B.toString(),S):(S,ee)=>a.nativeCallback(p,B.toString(),S,ee);if(z)return(L=z[B])===null||L===void 0?void 0:L.bind(z)}else{if(z)return(q=z[B])===null||q===void 0?void 0:q.bind(z);throw new ue(`"${p}" plugin is not implemented on ${l}`,Y.Unimplemented)}},f=z=>{let B;const L=(...q)=>{const D=b().then(S=>{const ee=m(S,z);if(ee){const te=ee(...q);return B=te==null?void 0:te.remove,te}else throw new ue(`"${p}.${z}()" is not implemented on ${l}`,Y.Unimplemented)});return z==="addListener"&&(D.remove=async()=>B()),D};return L.toString=()=>`${z.toString()}() { [capacitor code] }`,Object.defineProperty(L,"name",{value:z,writable:!1,configurable:!1}),L},x=f("addListener"),w=f("removeListener"),E=(z,B)=>{const L=x({eventName:z},B),q=async()=>{const S=await L;w({eventName:z,callbackId:S},B)},D=new Promise(S=>L.then(()=>S({remove:q})));return D.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await q()},D},U=new Proxy({},{get(z,B){switch(B){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return h?E:x;case"removeListener":return w;default:return f(B)}}});return i[p]=U,o.set(p,{name:p,proxy:U,platforms:new Set([...Object.keys(y),...h?[l]:[]])}),U};return a.convertFileSrc||(a.convertFileSrc=p=>p),a.getPlatform=n,a.handleError=d,a.isNativePlatform=r,a.isPluginAvailable=s,a.registerPlugin=v,a.Exception=ue,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},xt=e=>e.Capacitor=bt(e),T=xt(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),de=T.registerPlugin;class ge{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let i=!1;this.listeners[t]||(this.listeners[t]=[],i=!0),this.listeners[t].push(a);const r=this.windowListeners[t];r&&!r.registered&&this.addWindowListener(r),i&&this.sendRetainedArgumentsForEvent(t);const s=async()=>this.removeListener(t,a);return Promise.resolve({remove:s})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,i){const n=this.listeners[t];if(!n){if(i){let r=this.retainedEventArguments[t];r||(r=[]),r.push(a),this.retainedEventArguments[t]=r}return}n.forEach(r=>r(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:i=>{this.notifyListeners(a,i)}}}unimplemented(t="not implemented"){return new T.Exception(t,Y.Unimplemented)}unavailable(t="not available"){return new T.Exception(t,Y.Unavailable)}async removeListener(t,a){const i=this.listeners[t];if(!i)return;const n=i.indexOf(a);this.listeners[t].splice(n,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(i=>{this.notifyListeners(t,i)}))}}const _e=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Ee=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class kt extends ge{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(i=>{if(i.length<=0)return;let[n,r]=i.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");n=Ee(n).trim(),r=Ee(r).trim(),a[n]=r}),a}async setCookie(t){try{const a=_e(t.key),i=_e(t.value),n=t.expires?`; expires=${t.expires.replace("expires=","")}`:"",r=(t.path||"/").replace("path=",""),s=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${i||""}${n}; path=${r}; ${s};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}de("CapacitorCookies",{web:()=>new kt});const wt=async e=>new Promise((t,a)=>{const i=new FileReader;i.onload=()=>{const n=i.result;t(n.indexOf(",")>=0?n.split(",")[1]:n)},i.onerror=n=>a(n),i.readAsDataURL(e)}),_t=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(n=>n.toLocaleLowerCase()).reduce((n,r,s)=>(n[r]=e[t[s]],n),{})},Et=(e,t=!0)=>e?Object.entries(e).reduce((i,n)=>{const[r,s]=n;let u,d;return Array.isArray(s)?(d="",s.forEach(o=>{u=t?encodeURIComponent(o):o,d+=`${r}=${u}&`}),d.slice(0,-1)):(u=t?encodeURIComponent(s):s,d=`${r}=${u}`),`${i}&${d}`},"").substr(1):null,zt=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),n=_t(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(n.includes("application/x-www-form-urlencoded")){const r=new URLSearchParams;for(const[s,u]of Object.entries(e.data||{}))r.set(s,u);a.body=r.toString()}else if(n.includes("multipart/form-data")||e.data instanceof FormData){const r=new FormData;if(e.data instanceof FormData)e.data.forEach((u,d)=>{r.append(d,u)});else for(const u of Object.keys(e.data))r.append(u,e.data[u]);a.body=r;const s=new Headers(a.headers);s.delete("content-type"),a.headers=s}else(n.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class It extends ge{async request(t){const a=zt(t,t.webFetchExtra),i=Et(t.params,t.shouldEncodeUrlParams),n=i?`${t.url}?${i}`:t.url,r=await fetch(n,a),s=r.headers.get("content-type")||"";let{responseType:u="text"}=r.ok?t:{};s.includes("application/json")&&(u="json");let d,o;switch(u){case"arraybuffer":case"blob":o=await r.blob(),d=await wt(o);break;case"json":d=await r.json();break;case"document":case"text":default:d=await r.text()}const v={};return r.headers.forEach((p,y)=>{v[y]=p}),{data:d,headers:v,status:r.status,url:r.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}de("CapacitorHttp",{web:()=>new It});var ze;(function(e){e.Dark="DARK",e.Light="LIGHT",e.Default="DEFAULT"})(ze||(ze={}));var Ie;(function(e){e.StatusBar="StatusBar",e.NavigationBar="NavigationBar"})(Ie||(Ie={}));class Bt extends ge{async setStyle(){this.unavailable("not available for web")}async setAnimation(){this.unavailable("not available for web")}async show(){this.unavailable("not available for web")}async hide(){this.unavailable("not available for web")}}de("SystemBars",{web:()=>new Bt});var Be;(function(e){e[e.Sunday=1]="Sunday",e[e.Monday=2]="Monday",e[e.Tuesday=3]="Tuesday",e[e.Wednesday=4]="Wednesday",e[e.Thursday=5]="Thursday",e[e.Friday=6]="Friday",e[e.Saturday=7]="Saturday"})(Be||(Be={}));const K=de("LocalNotifications",{web:()=>ht(()=>import("./web-Cq1LmRMQ.js"),[]).then(e=>new e.LocalNotificationsWeb)}),Ae=39.9334,$e=32.8597,Se=3,Z=[{key:"Fajr",name:"İmsak",icon:"🌙"},{key:"Sunrise",name:"Güneş",icon:"🌅"},{key:"Dhuhr",name:"Öğle",icon:"☀️"},{key:"Asr",name:"İkindi",icon:"🌤️"},{key:"Maghrib",name:"Akşam",icon:"🌆"},{key:"Isha",name:"Yatsı",icon:"🌃"}],W=[{id:"haci-ruslan",name:"Hacı Ruslan",desc:"Hacı Ruslan sesi ile ezan",file:"/assets/audio/Haci Ruslan.mp3"},{id:"rahim-muazzinzade",name:"Rahim Müəzzinzade",desc:"Rahim Müəzzinzade sesi ile ezan",file:"/assets/audio/Rahim Muazzinzade.mp3"}];let A=localStorage.getItem("selectedEzan")||"haci-ruslan",H=localStorage.getItem("abdestReminder")!=="false",M=localStorage.getItem("ezanAutoPlay")!=="false",G=null,I=null,C=!1,Te={},Le={};function At(){const e=new Date,t=yt(e);if(t){const n=ke(e,Ae,$e,Se);return t.timings.Sunset=n.Sunset,t.timings.Midnight=n.Midnight,{timings:t.timings,hijri:t.hijri,date:e,isStatic:!0}}const a=ke(e,Ae,$e,Se),i=ut(e);return{timings:a,hijri:i,date:e,isStatic:!1}}function Ne(e){const t=new Date,a=t.getHours()*60+t.getMinutes();for(const i of Z){const n=e[i.key];if(!n||n==="--:--")continue;const[r,s]=n.split(":").map(Number),u=r*60+s;if(u>a)return{...i,time:n,prayerMinutes:u}}return{...Z[0],time:e.Fajr,prayerMinutes:1440}}function Fe(e){const t=new Date,[a,i]=e.split(":").map(Number);let n=a*60+i-(t.getHours()*60+t.getMinutes());n<0&&(n+=1440);const r=Math.floor(n/60),s=n%60,u=59-t.getSeconds();return`${String(r).padStart(2,"0")}:${String(s).padStart(2,"0")}:${String(u).padStart(2,"0")}`}function ve(e){const t=new Date,[a,i]=e.split(":").map(Number);let n=a*60+i-(t.getHours()*60+t.getMinutes());return n<0&&(n+=1440),n}function me(e){const t=W.find(a=>a.id===e);if(t){if(I&&C){I.pause(),C=!1,ne(),ie(!1);return}I&&(I.pause(),I=null),I=new Audio(t.file),I.volume=1,C=!0,I.play().catch(a=>{console.warn("Ezan playing failed:",a),C=!1}),I.addEventListener("ended",()=>{C=!1,ne(),ie(!1)}),I.addEventListener("timeupdate",()=>{$t(I)}),ne(),ie(!0)}}function Ge(){I&&(I.pause(),I.currentTime=0,I=null,C=!1,ne(),ie(!1))}function ne(){document.querySelectorAll(".ezan-play-btn").forEach(e=>{e.dataset.ezanId===A&&C?(e.innerHTML='<svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>',e.classList.add("playing")):(e.innerHTML='<svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><polygon points="5,3 19,12 5,21"/></svg>',e.classList.remove("playing"))})}function ie(e){const t=document.getElementById("ezan-waveform");t&&(e?t.classList.add("active"):t.classList.remove("active"))}function $t(e){const t=document.getElementById("ezan-progress-fill"),a=document.getElementById("ezan-time");if(t&&e.duration){const i=e.currentTime/e.duration*100;t.style.width=`${i}%`}if(a&&e.duration){const i=Me(e.currentTime),n=Me(e.duration);a.textContent=`${i} / ${n}`}}function Me(e){const t=Math.floor(e/60),a=Math.floor(e%60);return`${t}:${String(a).padStart(2,"0")}`}function St(e){const t=new Date,a=`${String(t.getHours()).padStart(2,"0")}:${String(t.getMinutes()).padStart(2,"0")}`;for(const i of Z){const n=e[i.key];if(!n||n==="--:--")continue;H&&ve(n)===15&&!Le[i.key]&&(Le[i.key]=!0,Notification.permission==="granted"&&new Notification("🚿 Abdest Vaktidir",{body:`${i.icon} ${i.name} namazına 15 dakika kaldı — Abdest almayı unutmayın!`,icon:"/assets/favicon.svg",tag:`abdest-${i.key}`}),pe(i));const r=["Fajr","Dhuhr","Maghrib"].includes(i.key);if(i.forceTest||M&&r&&n===a&&!Te[i.key]){i.forceTest||(Te[i.key]=!0);try{"Notification"in window&&Notification.permission==="granted"&&new Notification(`${i.icon} ${i.name} Vakti`,{body:`${i.name} vakti geldi — Ezan okunuyor`,icon:"/assets/favicon.svg",tag:`ezan-${i.key}`})}catch(s){console.warn("Background notification failed",s)}me(A)}}}function pe(e){var i;const t=document.getElementById("abdest-toast");t&&t.remove();const a=document.createElement("div");a.id="abdest-toast",a.className="abdest-toast abdest-toast--show",a.innerHTML=`
  ...
  `,document.body.appendChild(a),(i=document.getElementById("abdest-toast-close"))==null||i.addEventListener("click",()=>{a.classList.remove("abdest-toast--show"),setTimeout(()=>a.remove(),400)}),setTimeout(()=>{document.getElementById("abdest-toast")&&(a.classList.remove("abdest-toast--show"),setTimeout(()=>a.remove(),400))},1e4)}function Tt(e){G&&clearInterval(G);const t=()=>{const a=Ne(e),i=document.getElementById("prayer-countdown"),n=document.getElementById("prayer-next-name");i&&(i.textContent=Fe(a.time)),n&&(n.textContent=`${a.icon} ${a.name}`);const r=ve(a.time),s=document.getElementById("abdest-reminder-bar");s&&H&&(r<=15&&r>0?(s.style.display="flex",s.querySelector(".abdest-bar__text").textContent=`🚿 Abdest vaktidir — ${a.name} namazına ${r} dk kaldı`):s.style.display="none"),St(e)};t(),G=setInterval(t,1e3)}function Lt(e=24){return Array.from({length:e},(t,a)=>`<span class="waveform-bar" style="animation-delay:${(a*.08).toFixed(2)}s"></span>`).join("")}async function Ce(){var g;const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page">
  ...
  `,"Notification"in window&&Notification.permission==="default"&&Notification.requestPermission();const t=At(),a=t.timings,i=t.isStatic,n=t.hijri,r=Ne(a),s=`${n.day} ${n.monthNameAr} ${n.year} H.`,u=`${n.day} ${n.monthName} ${n.year}`,d=new Date().toLocaleDateString("tr-TR",{weekday:"long",day:"numeric",month:"long",year:"numeric"}),o=ve(r.time);e.innerHTML=`
    <div class="page">
      <!-- Abdest Reminder Bar -->
      ...
      <div class="prayer-hero">
        ...
        <div class="prayer-hero__countdown" id="prayer-countdown">${Fe(r.time)}</div>
      ...
      <div class="prayer-list">
        ${Z.map(l=>`
            <div class="prayer-item ${r.key===l.key?"prayer-item--active":""}">
              <div class="prayer-item__left">
                <div class="prayer-item__icon">${l.icon}</div>
                <span class="prayer-item__name">${l.name}</span>
              </div>
              <span class="prayer-item__time">${a[l.key]||"--:--"}</span>
            </div>
          `).join("")}
      </div>
      ...
      <div class="ezan-selector">
        <div class="section-header">
          ...
          <span class="badge badge--accent${M?"":" badge--muted"}" id="ezan-status">${C?"🔊 Çalıyor":M?"🔔 Aktif":"🔇 Sessiz"}</span>
        </div>
        ...
        <div class="ezan-player-card" id="ezan-player-card">
          <div class="ezan-player__header">
            ...
            <div>
              ...
              <div class="ezan-player__muezzin" id="ezan-current-name">${((g=W.find(l=>l.id===A))==null?void 0:g.name)||"Hacı Ruslan"}</div>
            </div>
          </div>
        ...
        <div class="ezan-options">
          ${W.map(l=>`
            <div class="ezan-option ${A===l.id?"ezan-option--active":""}" data-ezan="${l.id}" id="ezan-${l.id}">
              <div class="ezan-option__radio"></div>
              <div class="ezan-option__info">
                <div class="ezan-option__label">${l.name}</div>
                <div class="ezan-option__desc">${l.desc}</div>
              </div>
              <button class="ezan-play-btn" data-ezan-id="${l.id}" id="ezan-play-${l.id}" title="Dinle">
        ...
        <div class="ezan-settings-card">
          <div class="ezan-setting-row">
            ...
            <label class="toggle" for="toggle-ezan">
              <input type="checkbox" id="toggle-ezan" ${M?"checked":""} />
  ...
  `,e.querySelectorAll(".ezan-option").forEach(l=>{l.addEventListener("click",h=>{if(h.target.closest(".ezan-play-btn"))return;A=l.dataset.ezan,localStorage.setItem("selectedEzan",A),T.isNativePlatform()&&X(),e.querySelectorAll(".ezan-option").forEach(m=>m.classList.remove("ezan-option--active")),l.classList.add("ezan-option--active");const c=document.getElementById("ezan-current-name"),b=W.find(m=>m.id===A);c&&b&&(c.textContent=b.name)})}),e.querySelectorAll(".ezan-play-btn").forEach(l=>{l.addEventListener("click",h=>{h.stopPropagation();const c=l.dataset.ezanId;if(c!==A){A=c,localStorage.setItem("selectedEzan",A),T.isNativePlatform()&&X(),e.querySelectorAll(".ezan-option").forEach(w=>w.classList.remove("ezan-option--active"));const m=l.closest(".ezan-option");m&&m.classList.add("ezan-option--active");const f=document.getElementById("ezan-current-name"),x=W.find(w=>w.id===A);f&&x&&(f.textContent=x.name),Ge()}me(c);const b=document.getElementById("ezan-status");b&&(b.textContent=C?"🔊 Çalıyor":"🔇 Sessiz")})});const v=document.getElementById("toggle-ezan");v&&(v.checked=M,v.addEventListener("change",()=>{M=v.checked,localStorage.setItem("ezanAutoPlay",M?"true":"false"),T.isNativePlatform()&&X();const l=document.getElementById("ezan-status");l&&(M?(l.textContent=C?"🔊 Çalıyor":"🔔 Aktif",l.classList.remove("badge--muted")):(l.textContent="🔇 Sessiz",l.classList.add("badge--muted")))}));const p=document.getElementById("toggle-abdest");p&&p.addEventListener("change",()=>{H=p.checked,localStorage.setItem("abdestReminder",H?"true":"false"),T.isNativePlatform()&&X()}),Tt(a);const y=document.getElementById("btn-test-ezan");y&&y.addEventListener("click",async()=>{const l={key:"Fajr",name:"İmsak (Test)",icon:"🌙",forceTest:!0};if(T.isNativePlatform())try{const h=A==="haci-ruslan"?"haci_ruslan.mp3":"rahim_muazzinzade.mp3";await K.schedule({notifications:[{title:`${l.icon} ${l.name} Vakti`,body:"Bu bir test bildirimidir — Ezan okunuyor",id:9999,schedule:{at:new Date(new Date().getTime()+1e3)},sound:h,channelId:"ezan-channel"}]}),pe({name:"Test Bildirimi Gönderildi",icon:"✅"})}catch(h){console.error("Test bildirim hatasi:",h)}else{try{"Notification"in window&&(Notification.permission==="granted"?new Notification(`${l.icon} ${l.name} Vakti`,{body:"Bu bir test bildirimidir — Ezan okunuyor",icon:"/assets/favicon.svg",tag:"ezan-test"}):Notification.permission!=="denied"&&Notification.requestPermission())}catch(h){console.warn("Web Notification desteklenmiyor veya engellendi:",h)}pe({name:"Test (Ekranda)",icon:"✅"})}me(A)}),T.isNativePlatform()&&X()}async function X(){if(T.isNativePlatform())try{if((await K.checkPermissions()).display!=="granted"&&(await K.requestPermissions()).display!=="granted")return;const t=await K.getPending();if(t.notifications.length>0&&await K.cancel({notifications:t.notifications}),!M&&!H)return;const a=[],i=new Date;let n=1;const r=A==="haci-ruslan"?"haci_ruslan.mp3":"rahim_muazzinzade.mp3";T.getPlatform()==="android"&&await K.createChannel({id:"ezan-channel",name:"Ezan Bildirimleri",description:"Vakit girdiğinde ezan okur",importance:5,visibility:1,sound:r,vibration:!0}),Object.entries(Oe).forEach(([s,u])=>{const[d,o,v]=s.split("-").map(Number);new Date(d,o-1,v).getTime()<i.getTime()-864e5||["Fajr","Dhuhr","Asr","Maghrib","Isha"].forEach(y=>{const g=u[y];if(!g||g==="--:--")return;const[l,h]=g.split(":").map(Number),c=new Date(d,o-1,v,l,h,0),b=["Fajr","Dhuhr","Maghrib"].includes(y);if(M&&b&&c.getTime()>i.getTime()){const m=Z.find(f=>f.key===y);a.push({title:`${m?m.icon+" "+m.name:y} Vakti`,body:"Otomatik ezan okunuyor",id:n++,schedule:{at:c,allowWhileIdle:!0},sound:r,channelId:"ezan-channel"})}if(H){const m=new Date(c.getTime());if(m.setMinutes(m.getMinutes()-15),m.getTime()>i.getTime()){const f=Z.find(x=>x.key===y);a.push({title:"🚿 Abdest Vaktidir",body:`${f?f.icon+" "+f.name:y} namazına 15 dakika kaldı`,id:n++,schedule:{at:m,allowWhileIdle:!0}})}}})}),a.length>0&&(await K.schedule({notifications:a.slice(0,400)}),console.log(`[Native] Scheduled ${Math.min(a.length,400)} background notifications.`))}catch(e){console.error("Failed to schedule native notifications:",e)}}function Mt(){G&&(clearInterval(G),G=null),Ge()}let ae=null;async function Ct(){var t,a,i,n,r;const e=document.getElementById("page-content");e.innerHTML=`
  ...
  `;try{const[s,u]=await Promise.all([_("/community/announcements/"),_("/community/help-requests/")]);if(s._error)throw new Error("Duyurular yüklenemedi");e.innerHTML=`
      <div class="page">
        <!-- Community banner -->
          ...
          ${s.length>0?s.map(d=>`
            <div class="announcement-card">
              <div class="announcement-card__header">
                <span class="announcement-card__category" style="color:${d.categoryColor}">${d.category}</span>
                <div style="display:flex;align-items:center;gap:8px">
                  ${d.badge?`<span class="badge badge--accent">${d.badge}</span>`:""}
                  <span class="announcement-card__date">${d.date}</span>
                </div>
              </div>
              <h3 class="announcement-card__title">${d.title}</h3>
              <p class="announcement-card__body">${d.body}</p>
            </div>
        ...
        <div class="help-grid">
          ${u.length>0?u.map((d,o)=>`
            <div class="help-card" data-help-idx="${o}" id="help-card-${o}">
              <div class="help-card__icon">${d.icon}</div>
              <div class="help-card__title">${d.title}</div>
              <div class="help-card__count">${d.count}</div>
            </div>
    ...
    `,e.querySelectorAll(".help-card").forEach(d=>{d.addEventListener("click",()=>{const o=parseInt(d.dataset.helpIdx),v=u[o],p=document.getElementById("help-detail-panel"),y=document.getElementById("help-detail-title"),g=document.getElementById("help-detail-list");if(ae===o){p.style.display="none",ae=null,e.querySelectorAll(".help-card").forEach(l=>l.classList.remove("help-card--active"));return}ae=o,e.querySelectorAll(".help-card").forEach(l=>l.classList.remove("help-card--active")),d.classList.add("help-card--active"),y.textContent=`${v.icon} ${v.title}`,v.items&&v.items.length>0?g.innerHTML=v.items.map(l=>`
            <div class="help-detail__item">
              <div class="help-detail__item-info">
                <div class="help-detail__item-name">${l.name}</div>
                <div class="help-detail__item-desc">${l.desc}</div>
              </div>
              <div class="help-detail__item-date">${l.date}</div>
            </div>
          `).join(""):g.innerHTML='<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:13px;">Bu kategoride aktif talep yok.</div>',p.style.display="block",p.scrollIntoView({behavior:"smooth",block:"nearest"})})}),(t=document.getElementById("help-detail-close"))==null||t.addEventListener("click",()=>{document.getElementById("help-detail-panel").style.display="none",ae=null,e.querySelectorAll(".help-card").forEach(d=>d.classList.remove("help-card--active"))}),(a=document.getElementById("live-placeholder"))==null||a.addEventListener("click",()=>{document.getElementById("live-player-container").innerHTML=`
      ...
      `}),(i=document.getElementById("show-help-form-btn"))==null||i.addEventListener("click",()=>{if(!$()){window.location.hash="auth";return}const d=document.getElementById("help-form-container"),o=document.getElementById("show-help-form-btn");d.style.display==="none"?(d.style.display="block",o.textContent="✕ Formu Kapat"):(d.style.display="none",o.textContent="➕ Yeni Yardım Talebi Oluştur")}),(n=document.getElementById("help-category"))==null||n.addEventListener("change",d=>{const o=d.target.value;document.getElementById("blood-group-field").style.display=o==="Kan Bağışı"?"block":"none";const v=document.getElementById("help-title"),p=document.getElementById("help-desc"),y=v.previousElementSibling;o==="Kan Bağışı"?(y.textContent="Başlık / Ad Soyad",v.placeholder="Örn: Ahmet Yılmaz acil kan arıyor",p.placeholder="Hastanın yattığı hastane, aciliyet durumu vb. detaylar..."):o==="İş İlanları"?(y.textContent="İş Pozisyonu",v.placeholder="Örn: Tecrübeli Muhasebeci Aranıyor",p.placeholder="İşin tanımı, aranan nitelikler, çalışma koşulları..."):o==="Öğrenci Desteği"?(y.textContent="Destek Türü",v.placeholder="Örn: Üniversite öğrencisine burs veya laptop ihtiyacı",p.placeholder="Öğrencinin okuduğu bölüm, başarı durumu, ihtiyaç detayı..."):(y.textContent="Talebin Kısa Özeti",v.placeholder="Örn: Temel erzak ihtiyacı olan aile",p.placeholder="Yardım talebinizle ilgili detaylı açıklama...")}),(r=document.getElementById("help-request-form"))==null||r.addEventListener("submit",async d=>{d.preventDefault();const o=document.getElementById("help-submit-btn");o.disabled=!0,o.textContent="Gönderiliyor...";const v=document.getElementById("help-category").value;let p=document.getElementById("help-title").value,y=document.getElementById("help-desc").value;if(v==="Kan Bağışı"){const l=document.getElementById("help-blood-group").value;p=`[${l}] ${p}`,y=`Aranan kan grubu: ${l}. ${y}`}(await _("/community/help-requests/create/",{method:"POST",body:{category:v,name:p,desc:y,contact_info:document.getElementById("help-contact").value}}))._error?(k("Talep gönderilirken hata oluştu.","error"),o.disabled=!1,o.textContent="Talebi Gönder"):(k("Talebiniz başarıyla yayınlandı!","success"),d.target.reset(),setTimeout(()=>window.location.reload(),2e3))})}catch{k("Bağlantı hatası: Veriler yüklenemedi.","error"),e.innerHTML=`
  ...
  `;try{let[n,r]=await Promise.all([_("/ilim/hadith/today/"),_("/ilim/questions/")]);n._error&&(n={text:"İlim talep etmek her Müslüman'a farzdır.",source:"Hz. Muhammed (s.a.a)",reference:"Usul-u Kâfi, c.1, s.30"}),r._error&&(r=[]),e.innerHTML=`
      <div class="page">
        ...
        <div class="section-header">
          ...
          <span class="badge badge--primary">${je.length} eser</span>
        </div>
    ...
    `,(t=document.getElementById("ask-form"))==null||t.addEventListener("submit",async s=>{s.preventDefault();const u=document.getElementById("question-input"),d=document.getElementById("anonymous-check").checked,o=document.getElementById("private-check").checked;if(!u.value.trim())return;const v=document.getElementById("ask-submit-btn");v.disabled=!0,v.textContent="Gönderiliyor...",(await _("/ilim/questions/",{method:"POST",body:{text:u.value,is_anonymous:d,is_private:o},requireAuth:!0}))._error?(k("Soru gönderilirken bir hata oluştu.","error"),v.disabled=!1,v.textContent="Soruyu Gönder"):(k("Sorunuz başarıyla iletildi!","success"),u.value="",v.disabled=!1,v.textContent="Soruyu Gönder")}),e.querySelectorAll(".qa-toggle").forEach(s=>{s.addEventListener("click",()=>{const u=s.dataset.qaTarget,d=document.getElementById(`qa-answer-${u}`),o=document.getElementById(`qa-item-${u}`),v=s.querySelector(".qa-item__chevron");d.style.display==="none"?(d.style.display="block",o.classList.remove("qa-item--collapsed"),o.classList.add("qa-item--expanded"),v&&(v.textContent="⌄")):(d.style.display="none",o.classList.add("qa-item--collapsed"),o.classList.remove("qa-item--expanded"),v&&(v.textContent="›"))})}),(a=document.getElementById("qa-show-more"))==null||a.addEventListener("click",()=>{e.querySelectorAll(".qa-item--hidden").forEach(s=>{s.classList.remove("qa-item--hidden"),s.style.animation="fadeIn 0.3s ease"}),document.getElementById("qa-show-more").remove()}),e.querySelectorAll(".library-item").forEach(s=>{s.addEventListener("click",()=>{const u=parseInt(s.dataset.bookIdx),d=document.getElementById(`book-detail-${u}`);if(J!==null&&J!==u){const o=document.getElementById(`book-detail-${J}`);o&&(o.style.display="none")}d.style.display==="none"?(d.style.display="block",J=u):(d.style.display="none",J=null)})}),(i=document.getElementById("hadith-share-btn"))==null||i.addEventListener("click",async()=>{const s={title:"Günün Hadisi — Ehli-Beyt Ankara",text:`${n.text}
— ${n.source}
(${n.reference})`};if(navigator.share)try{await navigator.share(s)}catch{}else await navigator.clipboard.writeText(s.text),k("Hadis panoya kopyalandı!","success")})}catch{e.innerHTML='<div class="page text-center"><p class="text-danger">Bağlantı hatası: İçerikler yüklenemedi.</p></div>'}}async function qt(){const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page" style="text-align:center;padding:40px;">
      <div class="skeleton" style="width:100%;height:200px;border-radius:12px;margin-bottom:20px;"></div>
    </div>
  `;try{const t=await _("/bagis/campaigns/");if(t._error)throw new Error("Kampanyalar yüklenemedi");e.innerHTML=`
      <div class="page">
        ...
        <div class="campaign-list">
          ${t.length>0?t.map(d=>{const o=Math.min(parseFloat(d.collected)/parseFloat(d.target)*100,100).toFixed(1);return`
              <div class="campaign-card">
                <div class="campaign-card__header">
                  <div class="campaign-card__icon">${d.icon}</div>
                  <div class="campaign-card__title">${d.title}</div>
                </div>
                <p class="campaign-card__desc">${d.description}</p>
                <div class="campaign-progress">
                  ...
                  <div class="campaign-progress__stats">
                    <span class="campaign-progress__collected">${parseFloat(d.collected).toLocaleString("tr-TR")} ₺ toplandı</span>
                    <span class="campaign-progress__target">Hedef: ${parseFloat(d.target).toLocaleString("tr-TR")} ₺</span>
                  </div>
                </div>
              </div>
          ...
          <form id="donation-form">
            <div class="form-group">
              ...
              <select id="campaign-select" class="form-select">
                ...
                ${t.map(d=>`<option value="${d.id}">${d.title}</option>`).join("")}
              </select>
            </div>
            <div class="form-group">
    ...
    `;let a="Humus",i=50;const n=e.querySelectorAll(".donation-type-card");n.forEach(d=>{d.addEventListener("click",()=>{n.forEach(o=>o.classList.remove("donation-type-card--active")),d.classList.add("donation-type-card--active"),a=d.dataset.type})});const r=e.querySelectorAll(".amount-btn"),s=document.getElementById("custom-amount-container"),u=document.getElementById("custom-amount-input");r.forEach(d=>{d.addEventListener("click",()=>{r.forEach(o=>o.classList.remove("amount-btn--active")),d.classList.add("amount-btn--active"),d.dataset.amount==="custom"?(s.style.display="block",u.focus(),i=0):(s.style.display="none",i=parseInt(d.dataset.amount))})}),document.getElementById("donation-form").addEventListener("submit",async d=>{d.preventDefault();let o=i;if(document.getElementById("custom-amount-btn").classList.contains("amount-btn--active")&&(o=parseInt(u.value),!o||o<=0)){k("Lütfen geçerli bir miktar girin.","error");return}const v=document.getElementById("campaign-select").value,p=document.getElementById("donor-name").value,y={amount:o,donation_type:a,donor_name:p||"Anonim"};v&&(y.campaign=parseInt(v));const g=document.getElementById("donate-submit-btn");g.disabled=!0,g.textContent="İşleniyor...",(await _("/bagis/donate/",{method:"POST",body:y,requireAuth:!0}))._error?(k("Bağış işlemi sırasında bir hata oluştu.","error"),g.disabled=!1,g.textContent="💳 Bağış Yap"):(k("Bağışınız başarıyla alındı. Allah kabul etsin!","success",5e3),setTimeout(()=>{window.location.reload()},2e3))})}catch{e.innerHTML='<div class="page text-center"><p class="text-danger">Bağlantı hatası: Kampanyalar yüklenemedi.</p></div>'}}const qe=[{icon:"🕋",title:"İmam Hüseyin (a.s) Ziyaretnamesi",desc:"Kerbela · Türkçe & Arapça",content:`<div class="ziyaret-content">
      ...
      <div class="page">
        ...
        <div class="section-header">
          ...
          <span class="badge badge--accent">${qe.length} rehber</span>
        </div>
  ...
  `,document.getElementById("kuran-back-btn").addEventListener("click",()=>{window.location.hash="ilim"});let a=[],i=[],n=-1,r=!1;const s=document.getElementById("kuran-audio"),u=document.getElementById("btn-play-pause");try{a=(await(await fetch("https://api.alquran.cloud/v1/surah")).json()).data;const h=document.getElementById("surah-selector");h.innerHTML=a.map(c=>`<option value="${c.number}">${c.number}. ${c.englishName} (${c.name})</option>`).join(""),document.getElementById("kuran-initial-loader").style.display="none",document.getElementById("kuran-content").style.display="block",h.addEventListener("change",c=>{d(parseInt(c.target.value))}),s.addEventListener("ended",()=>{n<i.length-1?o(n+1):v()}),s.addEventListener("play",()=>{r=!0,p(),y(n)}),s.addEventListener("pause",()=>{r=!1,p(),y(n)}),u.addEventListener("click",()=>{n===-1?i.length>0&&o(0):r?s.pause():s.play()}),document.getElementById("btn-prev-ayah").addEventListener("click",()=>{n>0&&o(n-1)}),document.getElementById("btn-next-ayah").addEventListener("click",()=>{n<i.length-1&&o(n+1)}),d(1)}catch(g){console.error("Kuran loading error",g),e.innerHTML=`<div class="page text-center"><p class="text-danger">Kur'an verileri yüklenemedi. Lütfen internet bağlantınızı kontrol edin.</p></div>`}async function d(g){v(),n=-1,document.getElementById("kuran-player").classList.remove("visible");const l=document.getElementById("verses-list");l.innerHTML='<div class="kuran-loading"><div class="kuran-loading-spinner"></div><p>Ayetler yükleniyor...</p></div>';try{const c=await(await fetch(`https://api.alquran.cloud/v1/surah/${g}/editions/quran-uthmani,tr.diyanet,ar.alafasy`)).json(),b=c.data[0],m=c.data[1],f=c.data[2],x=a[g-1];document.getElementById("surah-ar-name").textContent=x.name,document.getElementById("surah-tr-name").textContent=x.englishNameTranslation,document.getElementById("surah-revelation").textContent=x.revelationType==="Meccan"?"Mekki":"Medeni",document.getElementById("surah-ayah-count").textContent=x.numberOfAyahs,document.getElementById("player-surah-name").textContent=x.englishName,i=b.ayahs.map((w,E)=>({numberInSurah:w.numberInSurah,textAr:w.text,textTr:m.ayahs[E].text,audio:f.ayahs[E].audio})),l.innerHTML=i.map((w,E)=>`
        <div class="ayah-card" id="ayah-card-${E}">
          <div class="ayah-header">
            <div class="ayah-number">${w.numberInSurah}</div>
            <button class="ayah-play-btn" data-idx="${E}">
              <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
            </button>
          </div>
      ...
      `).join(""),document.querySelectorAll(".ayah-play-btn").forEach(w=>{w.addEventListener("click",E=>{const U=parseInt(E.currentTarget.dataset.idx);o(U)})})}catch{l.innerHTML='<p class="text-danger" style="text-align:center">Ayetler yüklenemedi.</p>'}}function o(g){if(!i[g])return;n=g;const l=i[g];s.src=l.audio,s.play(),document.getElementById("kuran-player").classList.add("visible"),document.getElementById("player-ayah-number").textContent=`Ayet ${l.numberInSurah}`;const h=document.getElementById(`ayah-card-${g}`);if(h){const b=h.getBoundingClientRect().top+window.scrollY+-80;window.scrollTo({top:b,behavior:"smooth"})}}function v(){s.pause()}function p(){r?u.innerHTML='<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg>':u.innerHTML='<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>'}function y(g){if(document.querySelectorAll(".ayah-card").forEach(l=>l.classList.remove("playing")),g!==-1){const l=document.getElementById(`ayah-card-${g}`);l&&r&&l.classList.add("playing")}}}function Rt(){const e=document.getElementById("kuran-audio");e&&(e.pause(),e.src="")}function Kt(){const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page">
  ...
  `;try{const t=await _("/auth/profile/",{requireAuth:!0});if(t._error){window.location.hash="#auth";return}e.innerHTML=`
    ...
    `,document.getElementById("logout-btn").addEventListener("click",()=>{Pe(),k("Güle güle! Tekrar görüşmek üzere.","info"),window.location.hash="#auth"})}catch{e.innerHTML=`<div class="page text-center"><p class="text-danger">Bağlantı hatası: Profil yüklenemedi.</p>
  ...
  `;const t=document.getElementById("admin-dashboard-grid"),a=document.getElementById("admin-content-area"),i=document.getElementById("btn-back-to-menu"),n=document.getElementById("admin-title"),r=g=>{if(g==="dashboard")t.style.display="grid",a.style.display="none",i.style.display="none",n.textContent="⚙️ Yönetim Paneli";else{t.style.display="none",a.style.display="block",i.style.display="block";const l={questions:"🙋 Gelen Sorular",announce:"📢 Duyuru Yayınla",tours:"👥 Tur Kayıtları",tourmanage:"🕋 Turları Yönet",campaigns:"💎 Kampayaları Yönet"};n.textContent=l[g]||"Yönetim",["questions","announce","tours","tourmanage","campaigns"].forEach(h=>{document.getElementById(`tab-content-${h}`).style.display=h===g?"block":"none"}),window.scrollTo({top:0,behavior:"smooth"})}};e.querySelectorAll(".admin-module-card").forEach(g=>{g.addEventListener("click",()=>r(g.dataset.module))}),i.addEventListener("click",()=>r("dashboard")),await Promise.all([s(),u(),d(),o(),v()]);async function s(){const g=await _("/ilim/questions/unanswered/",{requireAuth:!0}),l=document.getElementById("tab-content-questions"),h=document.getElementById("stat-questions-label");if(h&&(h.textContent=g._error?"Hata":`${g.length} bekleyen`),g._error||g.length===0){l.innerHTML='<div class="card card--dark" style="padding:24px; text-align:center; color:var(--text-muted); font-size:14px;">🎉 Tüm sorular cevaplanmış! Bekleyen soru yok.</div>';return}l.innerHTML=g.map(c=>`
    ...
    `).join(""),l.querySelectorAll(".answer-btn").forEach(c=>{c.addEventListener("click",async()=>{const b=c.dataset.qid,m=document.getElementById(`answer-${b}`).value;if(!m.trim())return;if(c.disabled=!0,c.textContent="Gönderiliyor...",(await _(`/ilim/questions/${b}/answer/`,{method:"PATCH",body:{answer:m},requireAuth:!0}))._error)k("Cevap gönderilirken bir hata oluştu.","error"),c.disabled=!1,c.textContent="Cevapla ve Yayınla";else{k("Cevabınız başarıyla yayınlandı!","success"),c.textContent="Cevaplandı ✅",c.classList.add("btn--success");const x=document.getElementById(`q-card-${b}`);x.style.transition="all 0.5s ease",x.style.opacity="0",x.style.transform="translateY(-10px)",setTimeout(()=>{x.remove();const E=document.getElementById("tab-content-questions");E.querySelectorAll(".card").length===0&&(E.innerHTML='<div class="card card--dark" style="padding:24px; text-align:center; color:var(--text-muted); font-size:14px;">🎉 Tüm sorular cevaplanmış! Bekleyen soru yok.</div>')},500);const w=document.getElementById("stat-questions-label");if(w){const E=parseInt(w.textContent)||0;w.textContent=`${Math.max(0,E-1)} bekleyen`}}})})}async function u(){const g=document.getElementById("tab-content-announce");g.innerHTML=`
    ...
    `;async function l(){const h=await _("/community/announcements/"),c=document.getElementById("ann-list"),b=document.getElementById("stat-announce-label");b&&(b.textContent=h._error?"Hata":`${h.length} duyuru`),c.innerHTML=h.map(m=>`
        <div class="card card--dark" style="padding:14px; margin-bottom:10px;" id="ann-card-${m.id}">
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div style="flex:1; min-width:0;">
              <div style="display:flex; align-items:center; gap:6px; margin-bottom:4px;">
                <span style="font-size:10px; background:var(--bg-secondary); padding:2px 8px; border-radius:8px; color:${m.categoryColor};">${m.category}</span>
                ${m.badge?`<span class="badge badge--accent" style="font-size:9px;">${m.badge}</span>`:""}
              ...
              <div style="font-size:12px; color:var(--text-secondary); line-height:1.4; overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">${m.body}</div>
            </div>
            <button class="btn btn--danger btn--sm delete-ann-btn" data-id="${m.id}" style="padding:4px 10px; font-size:11px; margin-left:10px; flex-shrink:0;">🗑️ Sil</button>
          </div>
        </div>
      `).join(""),c.querySelectorAll(".delete-ann-btn").forEach(m=>{m.addEventListener("click",async()=>{if(!confirm("Bu duyuruyu kaldırmak istediğinize emin misiniz?"))return;const f=m.dataset.id;if(m.textContent="...",m.disabled=!0,(await _("/community/announcements/"+f+"/delete/",{method:"DELETE",requireAuth:!0}))._error)k("Duyuru silinemedi","error"),m.textContent="🗑️ Sil",m.disabled=!1;else{k("Duyuru başarıyla kaldırıldı","success");const w=document.getElementById("ann-card-"+f);w&&(w.style.transition="opacity 0.3s, transform 0.3s",w.style.opacity="0",w.style.transform="translateX(20px)",setTimeout(()=>{w.remove();const E=c.querySelectorAll(".card").length;document.getElementById("stat-announcements").textContent=E,E===0&&(c.innerHTML='<div class="card card--dark" style="padding:20px;text-align:center;color:var(--text-muted);font-size:14px;">Aktif duyuru yok.</div>')},300))}})})}document.getElementById("announcement-form").addEventListener("submit",async h=>{h.preventDefault();const c=document.getElementById("ann-submit-btn");c.disabled=!0,c.textContent="Yayınlanıyor...";const b={category:document.getElementById("ann-category").value,title:document.getElementById("ann-title").value,body:document.getElementById("ann-body").value,badge:document.getElementById("ann-badge").value||""};(await _("/community/announcements/create/",{method:"POST",body:b,requireAuth:!0}))._error?(k("Duyuru yayınlanırken bir hata oluştu.","error"),c.disabled=!1,c.textContent="📢 Duyuruyu Yayınla"):(k("Duyuru başarıyla yayınlandı!","success"),h.target.reset(),c.disabled=!1,c.textContent="📢 Duyuruyu Yayınla",await l())}),await l()}async function d(){const g=await _("/ziyaret/registrations/",{requireAuth:!0}),l=document.getElementById("tab-content-tours"),h=document.getElementById("stat-tours-label");if(h&&(h.textContent=g._error?"Hata":`${g.length} yeni kayıt`),g._error||g.length===0){l.innerHTML='<div class="card card--dark" style="padding:24px; text-align:center; color:var(--text-muted); font-size:14px;">Henüz tur kaydı bulunmuyor.</div>';return}l.innerHTML=`
    ...
    `}async function o(){const g=document.getElementById("tab-content-tourmanage"),l=`
      ...
      <div id="tm-list"></div>
    `;g.innerHTML=l;const h=async()=>{const c=await _("/ziyaret/admin/tours/",{requireAuth:!0}),b=document.getElementById("tm-list");if(c._error||c.length===0){b.innerHTML='<div style="text-align:center; color:var(--text-muted); font-size:14px;">Mevcut tur bulunamadı.</div>';return}b.innerHTML=c.map(m=>`
        <div class="card card--dark" style="padding:14px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
          <div>
            ...
            <div style="font-size:12px; color:var(--text-muted);">${m.date_start} - ${m.date_end} · ${m.price}</div>
          </div>
          <div style="display:flex; gap:6px;">
            <button class="btn btn--primary btn--sm edit-tour-btn" data-tour='${JSON.stringify(m).replace(/'/g,"&apos;")}' style="padding:4px 8px; font-size:11px;">Düzenle</button>
            <button class="btn btn--danger btn--sm delete-tour-btn" data-id="${m.id}" style="padding:4px 8px; font-size:11px;">Sil</button>
          </div>
        </div>
      `).join(""),b.querySelectorAll(".delete-tour-btn").forEach(m=>{m.addEventListener("click",async()=>{if(!confirm("Bu turu silmek istediğinize emin misiniz?"))return;const f=m.dataset.id;m.textContent="...",m.disabled=!0,(await _("/ziyaret/admin/tours/"+f+"/",{method:"DELETE",requireAuth:!0}))._error?(k("Tur silinemedi","error"),m.textContent="Sil",m.disabled=!1):(k("Tur başarıyla silindi","success"),h())})}),b.querySelectorAll(".edit-tour-btn").forEach(m=>{m.addEventListener("click",()=>{const f=JSON.parse(m.dataset.tour);document.getElementById("tm-id").value=f.id,document.getElementById("tm-title").value=f.title,document.getElementById("tm-start").value=f.date_start,document.getElementById("tm-end").value=f.date_end,document.getElementById("tm-duration").value=f.duration,document.getElementById("tm-price").value=f.price,document.getElementById("tm-seats").value=f.total_seats,document.getElementById("tm-status").value=f.status,document.getElementById("tm-color").value=f.status_color,document.getElementById("tm-form-title").textContent="Turu Düzenle",document.getElementById("tm-submit-btn").textContent="Güncelle",document.getElementById("tm-cancel-btn").style.display="block",window.scrollTo({top:0,behavior:"smooth"})})})};document.getElementById("tm-cancel-btn").addEventListener("click",()=>{document.getElementById("tour-manage-form").reset(),document.getElementById("tm-id").value="",document.getElementById("tm-form-title").textContent="Yeni Tur Ekle",document.getElementById("tm-submit-btn").textContent="Turu Ekle",document.getElementById("tm-cancel-btn").style.display="none"}),document.getElementById("tour-manage-form").addEventListener("submit",async c=>{c.preventDefault();const b=document.getElementById("tm-submit-btn");b.disabled=!0;const m=document.getElementById("tm-id").value,f=m?"PATCH":"POST",x=m?"/ziyaret/admin/tours/"+m+"/":"/ziyaret/admin/tours/";b.textContent=m?"Güncelleniyor...":"Ekleniyor...";const w={title:document.getElementById("tm-title").value,date_start:document.getElementById("tm-start").value,date_end:document.getElementById("tm-end").value,duration:document.getElementById("tm-duration").value,price:document.getElementById("tm-price").value,total_seats:parseInt(document.getElementById("tm-seats").value),status:document.getElementById("tm-status").value,status_color:document.getElementById("tm-color").value,is_active:!0};(await _(x,{method:f,body:w,requireAuth:!0}))._error?k(m?"Tur güncellenirken hata oluştu":"Tur eklenirken hata oluştu","error"):(k(m?"Tur başarıyla güncellendi!":"Tur başarıyla eklendi!","success"),c.target.reset(),document.getElementById("tm-cancel-btn").click(),await h()),b.disabled=!1,b.textContent=m?"Güncelle":"Turu Ekle"}),await h()}async function v(){const g=document.getElementById("tab-content-campaigns");g.innerHTML=`
      ...
      <div id="cm-list"></div>
    `;const l=async()=>{const h=await _("/bagis/admin/campaigns/",{requireAuth:!0}),c=document.getElementById("cm-list"),b=document.getElementById("stat-campaigns-label");if(h._error){c.innerHTML='<div style="text-align:center; color:var(--text-danger); font-size:14px;">Kampanyalar yüklenirken hata oluştu.</div>',b&&(b.textContent="Hata");return}const m=h.filter(f=>f.is_active).length;if(b&&(b.textContent=`${m} aktif kampanya`),h.length===0){c.innerHTML='<div style="text-align:center; color:var(--text-muted); font-size:14px;">Henüz kampanya bulunmuyor.</div>';return}c.innerHTML=h.map(f=>`
        <div class="card card--dark" style="padding:14px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
          <div style="flex:1;">
            <div style="display:flex; align-items:center; gap:8px;">
              <span style="font-size:18px;">${f.icon}</span>
              <div style="font-weight:bold; font-size:14px;">${f.title}</div>
            </div>
            <div style="font-size:12px; color:var(--text-muted); margin-top:4px;">
              ${parseFloat(f.collected_amount).toLocaleString("tr-TR")} / ${parseFloat(f.target_amount).toLocaleString("tr-TR")} ₺
              <span style="margin-left:8px; color:${f.is_active?"var(--color-primary)":"var(--text-danger)"}; font-size:10px;">
                ● ${f.is_active?"Aktif":"Kapalı"}
              </span>
            </div>
          </div>
          <div style="display:flex; gap:6px;">
            <button class="btn btn--primary btn--sm edit-campaign-btn" data-campaign='${JSON.stringify(f).replace(/'/g,"&apos;")}' style="padding:4px 8px; font-size:11px;">Düzenle</button>
            <button class="btn btn--danger btn--sm delete-campaign-btn" data-id="${f.id}" style="padding:4px 8px; font-size:11px;">Sil</button>
          </div>
        </div>
      `).join(""),c.querySelectorAll(".delete-campaign-btn").forEach(f=>{f.addEventListener("click",async()=>{if(!confirm("Bu kampanyayı silmek istediğinize emin misiniz?"))return;const x=f.dataset.id;f.textContent="...",f.disabled=!0,(await _("/bagis/admin/campaigns/"+x+"/",{method:"DELETE",requireAuth:!0}))._error?(k("Kampanya silinemedi","error"),f.textContent="Sil",f.disabled=!1):(k("Kampanya silindi","success"),l())})}),c.querySelectorAll(".edit-campaign-btn").forEach(f=>{f.addEventListener("click",()=>{const x=JSON.parse(f.dataset.campaign);document.getElementById("cm-id").value=x.id,document.getElementById("cm-title").value=x.title,document.getElementById("cm-desc").value=x.description,document.getElementById("cm-target").value=parseInt(x.target_amount),document.getElementById("cm-collected").value=parseInt(x.collected_amount),document.getElementById("cm-icon").value=x.icon,document.getElementById("cm-active").value=x.is_active.toString(),document.getElementById("cm-form-title").textContent="Kampanyayı Düzenle",document.getElementById("cm-submit-btn").textContent="Güncelle",document.getElementById("cm-cancel-btn").style.display="block",window.scrollTo({top:0,behavior:"smooth"})})})};document.getElementById("cm-cancel-btn").addEventListener("click",()=>{document.getElementById("campaign-manage-form").reset(),document.getElementById("cm-id").value="",document.getElementById("cm-form-title").textContent="Yeni Kampanya Ekle",document.getElementById("cm-submit-btn").textContent="Kampanyayı Ekle",document.getElementById("cm-cancel-btn").style.display="none"}),document.getElementById("campaign-manage-form").addEventListener("submit",async h=>{h.preventDefault();const c=document.getElementById("cm-submit-btn");c.disabled=!0;const b=document.getElementById("cm-id").value,m=b?"PATCH":"POST",f=b?"/bagis/admin/campaigns/"+b+"/":"/bagis/admin/campaigns/";c.textContent=b?"Güncelleniyor...":"Ekleniyor...";const x={title:document.getElementById("cm-title").value,description:document.getElementById("cm-desc").value,target_amount:parseFloat(document.getElementById("cm-target").value),collected_amount:parseFloat(document.getElementById("cm-collected").value),icon:document.getElementById("cm-icon").value,is_active:document.getElementById("cm-active").value==="true"};(await _(f,{method:m,body:x,requireAuth:!0}))._error?k(b?"Kampanya güncellenirken hata":"Kampanya eklenirken hata","error"):(k(b?"Kampanya güncellendi!":"Kampanya eklendi!","success"),h.target.reset(),document.getElementById("cm-cancel-btn").click(),await l()),c.disabled=!1,c.textContent=b?"Güncelle":"Kampanyayı Ekle"}),await l()}const p=await _("/community/announcements/"),y=document.getElementById("stat-announce-label");y&&(y.textContent=p._error?"Hata":`${p.length} duyuru`)}let O="prayer";function F(e){O==="prayer"?Mt():O==="kuran"&&Rt(),O=e,window.location.hash=e,tt(O,F),window.scrollTo(0,0);const t=document.querySelector(".bottom-nav"),a=["kvkk","auth","profile","admin","kuran"];switch(t&&(t.style.display=a.includes(e)?"none":"flex"),e){case"prayer":Ce();break;case"community":Ct();break;case"ilim":jt();break;case"bagis":qt();break;case"ziyaret":Pt();break;case"kuran":Ht();break;case"kvkk":Kt();break;case"auth":if($()){F("profile");return}Nt();break;case"profile":if(!$()){F("auth");return}Ft();break;case"admin":if(!$()){F("auth");return}Gt();break;default:Ce()}}window.addEventListener("hashchange",()=>{const e=window.location.hash.replace("#","")||"prayer";e!==O&&F(e)});function Yt(){He();const e=window.location.hash.replace("#","");e&&["prayer","community","ilim","bagis","ziyaret","kuran","kvkk","auth","profile","admin"].includes(e)&&(O=e),F(O)}async function Ye(){if("serviceWorker"in navigator&&"PushManager"in window&&$())try{const e=await navigator.serviceWorker.ready;if(await Notification.requestPermission()==="granted"){const a=Zt("BEl62iUYgUivxIkv69yViEuiBIa-Ib9-SkvMeAtA3LFgDzkrxZJjSgSnfckjBJuB-3qOX7j30CG3EMGWpncwYkU"),i=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:a});await apiFetch("/auth/webpush/subscribe/",{method:"POST",body:{status_type:"subscribe",subscription:i,browser:navigator.userAgent.includes("Chrome")?"chrome":"other",user_agent:navigator.userAgent},requireAuth:!0})}}catch(e){console.error("Push registration failed:",e)}}function Zt(e){const t="=".repeat((4-e.length%4)%4),a=(e+t).replace(/-/g,"+").replace(/_/g,"/"),i=window.atob(a),n=new Uint8Array(i.length);for(let r=0;r<i.length;++r)n[r]=i.charCodeAt(r);return n}window.addEventListener("authChange",()=>{He(),Ye()});function De(){Yt(),setTimeout(Ye,3e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",De):De();export{ge as W};
